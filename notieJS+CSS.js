...
// @language ECMASCRIPT2017
// @language_out ECMASCRIPT5_STRICT
// @export var notie
// ==/ClosureCompiler==
notie=(function(document, Object){"use strict";
// load in the CSS:
document.getElementsByTagName("html")[0].insertAdjacentHTML("beforeend", "<style>.notie-container{font-size:1.6rem;font-size:calc(1.042vw + 1.042vh + .36rem);height:auto;left:0;position:fixed;text-align:center;width:100%;z-index:2;box-sizing:border-box;-o-box-shadow:0 0 5px 0 rgba(0,0,0,.5);-ms-box-shadow:0 0 5px 0 rgba(0,0,0,.5);box-shadow:0 0 5px 0 rgba(0,0,0,.5)}.notie-background-success{background-color:#57bf57}.notie-background-warning{background-color:#d6a14d}.notie-background-error{background-color:#e1715b}.notie-background-info{background-color:#4d82d6}.notie-background-neutral{background-color:#a0a0a0}.notie-background-overlay{background-color:#fff}.notie-textbox{color:#fff;padding:20px}.notie-textbox-inner{margin:0 auto;max-width:900px}.notie-overlay{height:100%;left:0;opacity:0;position:fixed;top:0;width:100%;z-index:1}.notie-button{cursor:pointer}.notie-button,.notie-element{color:#fff;padding:10px}.notie-element-half{width:50%}.notie-element-half,.notie-element-third{display:inline-block;box-sizing:border-box}.notie-element-third{width:33.3333%}.notie-alert{cursor:pointer}.notie-input-field{background-color:#fff;border:0;font-family:inherit;font-size:inherit;outline:0;padding:10px;text-align:center;width:100%;box-sizing:border-box}.notie-select-choice-repeated{border-bottom:1px solid hsla(0,0%,100%,.2);box-sizing:border-box}.notie-date-selector-inner{margin:0 auto;max-width:900px;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;-o-user-select:none;user-select:none}.notie-date-selector-inner [contenteditable],.notie-date-selector-inner [contenteditable]:focus{outline-width:0}.notie-date-selector-up{-webkit-transform:rotate(180deg);transform:rotate(180deg)}</style>")
...
const /**@nocollapse*/ positions = {
  top: 'top',
  bottom: 'bottom'
}

let /**@nocollapse*/ options = {
  alertTime: 3,
...
const /**@nocollapse*/ setOptions = newOptions => {
  options = {
    ...options,
    ...newOptions,
...
const addToDocument = (element, position) => {
  ...
  if (element.listener) addEventListener('keydown', element.listener)
...
const removeFromDocument = (id, position) => {
  ...
  if (element.listener) removeEventListener('keydown', element.listener)
...
const alert = ({
  "type": type = 4,
  "text": text,
  "time": time = options.alertTime,
  "stay": stay = false,
  "position": position = options.positions.alert || positions.top
}) => {
  blur()
  hideAlerts()
...
const force = ({
  ...
  "callback": callback,
  "position": position = options.positions.force || positions.top
}, callbackArg) => {
  blur()
  hideAlerts()
...
const confirm = ({
  ...
  "position": position = options.positions.confirm || positions.top
...
const input = (settings, submitCallbackArg, cancelCallbackArg) => {
  const {
    "text": text,
    ...
    "submitCallback": submitCallback,
    "cancelCallback": cancelCallback,
    "position": position = options.positions.input || positions.top
  } = settings;
  ...
  const availableOptions = Object_setPrototypeOf({
      "autocapitalize": "none",
      "autocomplete": "off",
      "autocorrect": "off",
      "autofocus": true,
  ...
  var priorPrototype = Object.getPrototypeOf(settings);
  Object_setPrototypeOf(settings, availableOptions);

  for (var key in availableOptions) {
      // allow default pass-through in a very tiny performant manner
      elementInput.setAttribute(key, ""+settings[key]);
  }
  
  Object_setPrototypeOf(settings, priorPrototype);
...
const select = ({
  ...
  "cancelCallback": cancelCallback,
  "choices": choices,
  "position": position = options.positions.select || positions.top
}, cancelCallbackArg) => {
  blur()
  hideAlerts()
  ...
  choices.forEach(({ "type": type = 1, "text": text, "handler": handler }, index) => {
    ...
    if (options.classes.selectChoice) elementChoice.classList.add(options.classes.selectChoice)
...
const date = ({
  ...
  "submitCallback": submitCallback,
  "cancelCallback": cancelCallback,
  "position": position = options.positions.date || positions.top
}, submitCallbackArg, cancelCallbackArg) => {
  blur()
  hideAlerts()
